...
<Menu {position} absoluteClass="right-2" className="py-2 w-56 h-[342px]">
  ...
  <div class="flex w-full" on:click={() => {linkToggle.toggleButton()}}>
    <div class="flex relative w-full items-center h-8 px-1.5 cursor-pointer select-none hover:bg-buttonblue-100 {isHoverButtonLink ? '' : 'active:bg-buttonblue-200'}">
      ...
      <Toggle toggle={false} bind:this={linkToggle} on:mouseenter={() => isHoverButtonLink = true} on:mouseleave={() => isHoverButtonLink = false}/>
    </div>
  </div>
  ...
  <div class="flex w-full" on:click={() => {linkToggle.toggleButton()}}>
    <div class="flex relative w-full items-center h-8 px-1.5 select-none gap-1">
      ...
      <Button 
        ...
        tooltipClass="mt-1"
      />
      <div class="text-left text-sm text-buttonblue-800">100 %</div>
      <Button 
        className="h-6 w-6" 
        icon="add"
        tooltip="Redo" 
  ...
  <SidebarHeading title="Horizontal placement" className="pl-[14px] pt-0.5"/>
  <div class="flex flex-col gap-1 pb-1">
    <CommentOption title="Left" icon="custom_placement_left" className="mt-0"/>
    <CommentOption title="Center" icon="custom_placement_center" active={true} className="mt-0"/>
    <CommentOption title="Right" icon="custom_placement_right" className="mt-0"/>
  </div>

  <div class="h-px w-full bg-bordergray mt-2 mb-3"></div>

  <SidebarHeading title="Vertical placement" className="pl-[14px] pt-0.5"/>
  <div class="flex flex-col gap-1 pb-1">
